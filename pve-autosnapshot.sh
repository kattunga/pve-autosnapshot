...
get_vms ()
{
    ...
    then
        ...
        VM_LIST_LXC=$( pvesh get /nodes/${NODE_ID}/lxc --noborder=1 --noheader=1 2> $ERR_FILE | awk '{print$2}' )
        check_for_error
        if [ "${VM_ID}" != "" ]; then
    ...
    then
        ...
        VM_LIST_QEMU=$( pvesh get /nodes/${NODE_ID}/qemu --noborder=1 --noheader=1 2> $ERR_FILE | awk '{print$2}' )
        check_for_error
        if [ "${VM_ID}" != "" ]; then
...
snapshot_vm ()
{
    ...
    then
        ...
        then
            log_error "wrong --snap-retain value, remember to add the 'ago' word. ex: 1 day ago or use the minus sign: -2 days"
        fi

        [ $DEBUG = true ] && log "vm ${vm} get snapshot list"
        SNAP_LIST=$( pvesh get /nodes/${NODE_ID}/${type}/${vm}/snapshot --output-format yaml 2> $ERR_FILE | grep "name: ${SNAP_PREF}" | awk '{print$2}' | sort | head -n -1 )
        check_for_error
...
# create isolated tmp directory. must happen before the first log_error:
# $ERR_FILE and the log file live in here
mkdir -p $TMP_PATH > /dev/null 2>&1
[ $DEBUG = true ] && log "TMP_PATH = $TMP_PATH"

# register exit/clean up tasks. the lock file is NOT registered yet, on purpose:
# it is not ours until the check below passes, and removing it here would delete
# the lock of the process that is actually running
on_exit_run	log "end-------------------------------------------------------------"
...
if [ -e "$LOCK_FILE" ]
then
	log_error "autosnapshot is currently running."
fi
